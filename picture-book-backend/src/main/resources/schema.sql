...
CREATE TABLE IF NOT EXISTS biz_family (
    id                BIGINT AUTO_INCREMENT PRIMARY KEY,
    family_name       VARCHAR(64) NOT NULL COMMENT '家长昵称',
    parent_name       VARCHAR(64) COMMENT '家长真实姓名',
    username          VARCHAR(64) NOT NULL UNIQUE COMMENT '登录账号',
    password          VARCHAR(128) NOT NULL COMMENT 'BCrypt加密密码',
    phone             VARCHAR(32) COMMENT '手机号',
    avatar            VARCHAR(255) COMMENT '头像URL',
    member_level      VARCHAR(32) DEFAULT 'free' COMMENT 'free/growth/family',
    member_status     VARCHAR(16) DEFAULT '0' COMMENT '0=未开通 1=已开通 2=已过期',
    member_start      TIMESTAMP COMMENT '会员开始时间',
    member_expire     TIMESTAMP COMMENT '会员到期时间',
    whitelist_themes  VARCHAR(512) COMMENT '内容白名单主题，逗号分隔',
    blacklist_themes  VARCHAR(512) COMMENT '内容黑名单主题，逗号分隔',
    daily_limit_min   INT DEFAULT 30 COMMENT '每日阅读上限(分钟)',
    single_limit_min  INT DEFAULT 15 COMMENT '单次阅读上限(分钟)',
    anti_addiction_flag VARCHAR(8) DEFAULT '0' COMMENT '防沉迷提醒 0=关 1=开',
...
CREATE TABLE IF NOT EXISTS biz_child_profile (
    id              BIGINT AUTO_INCREMENT PRIMARY KEY,
    family_id       BIGINT NOT NULL COMMENT '绑定的家长账号ID（孩子只能由家长创建）',
    username        VARCHAR(64) UNIQUE COMMENT '孩子登录账号',
    password        VARCHAR(128) COMMENT 'BCrypt加密密码',
    child_name      VARCHAR(64) COMMENT '孩子昵称',
    avatar          VARCHAR(255) COMMENT '孩子头像URL',
    birth_date      DATE,
    ...
    status          VARCHAR(8) DEFAULT '0' COMMENT '0=正常 1=停用',
